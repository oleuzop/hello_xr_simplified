...
struct D3D11GraphicsPlugin : public IGraphicsPlugin {
    ...
    D3D11GraphicsPlugin()
        : m_clearColor({ 0.184313729f, 0.309803933f, 0.309803933f, 1.0f }) {}
    ...
    void InitializeDevice(XrInstance instance, XrSystemId systemId) override {
        ...
        CHECK_XRCMD(xrGetInstanceProcAddr(instance, "xrGetD3D11GraphicsRequirementsKHR", reinterpret_cast<PFN_xrVoidFunction*>(&pfnGetD3D11GraphicsRequirementsKHR)));
        ...
        featureLevels.erase(
            std::remove_if(featureLevels.begin(), featureLevels.end(), [&](D3D_FEATURE_LEVEL fl) { return fl < graphicsRequirements.minFeatureLevel; }),
            featureLevels.end());
    ...
    void RenderView(
        ...
        CD3D11_VIEWPORT viewport(
            (float)layerView.subImage.imageRect.offset.x,
...
std::shared_ptr<IGraphicsPlugin> CreateGraphicsPlugin() {
    return std::make_shared<D3D11GraphicsPlugin>();
}
